...
def H_back(L,T):
    ...
    #One can estimate the equilibrium heating rate by 
    #equating the conductive losses with 2/3 of the heating. 
    #(The radiative losses are about 1/3 of the heating).
    #Thus:
    # (2/3)H = (2/7) * k * T^(7/2) L^-2
    ...
    '''
    k = 8.12e-7 #in cgs (is Spitzer thermal conductivity coefficient) (𝐞𝐫𝐠 𝐬^{−𝟏} 𝐜𝐦^{−𝟏} 𝐊^{𝟕/2})
    cons = 0.4131267
    T2 = T**(7.0/2.0) # T in k
    L2 = L**2 # Half loop length in cm
    return cons * k * (T2/L2)

def H_back_loopTop(L,T):
    #function to estimate the required heating rate to mentain a loop-top temperature.
