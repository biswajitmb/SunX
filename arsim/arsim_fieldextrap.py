...
class fieldextrap(object): 
    def __init__(self,configfile=None):
        ...
        if NumCore > 0:
            ...
            if NumCore > self.cpu:
                print("%% simar_message: NumCore is exciding the maximum value. Set NumCore = "+str(cpu))
                NumCore = self.cpu 
            #print("%% simar_status: Parallel processing is using with no.cores = "+str(NumCore))
            #self.NumCore = NumCore
        elif NumCore == 0: print("%% simar_massege: Using single process!") 
        else: print("%% simar_Error: < NumCore > should be an integer, like 0,1,2,3... (0 indicated no parallel process)")
    ...
    def get_data(self):
        ...
        if os.path.isdir(data_dir) is False:raise Exception("%% simar_error : Directory not exist- "+data_dir)
        ...
        t = hmi_final_file["Start Time"].value[0].split(' ')
        t1 = t[1].split(':')
        ...
        aia_wl = aia['vso'][aia['vso']["Wavelength"][:, 0] == int(config['SelectEvent']['aia_filter'])* u.AA]
        ...
        t = aia_final_file["Start Time"].value[0].split(' ')
        t1 = t[1].split(':')
        afname = glob.glob(os.path.join(aia_dir,'aia_lev1_'+ind_2(config['SelectEvent']['aia_filter'])+'a_'+t[0][0:4]+'_'+t[0][5:7]+'_'+t[0][8:10]+'t'+t1[0]+'_'+t1[1]+'_'+t1[2].split('.')[0]+'_*_image_lev1.fits'))[0]
        afname = os.path.basename(afname)

        self.hfname = hfname
        self.afname = afname

    def data_processing(self,AIA_Image=None, HMI_Mag = None):
        '''
        ...
        m_registered.save(os.path.join(adir,'AIA_lev1.5_'+ind_4(self.config['SelectEvent']['aia_filter'])+'_'+format('%0.4d'%datetim.year)+format('%0.2d'%datetim.month)+format('%0.2d'%datetim.day)+'T'+format('%0.2d'%datetim.hour)+format('%0.2d'%datetim.minute)+format('%0.2d'%datetim.second)+'.fits'),overwrite='True')
        ...
        if self.config['SelectEvent']['remove_level1_data'] is True:
            os.remove(os.path.join(adir,self.afname))
            os.remove(os.path.join(hdir,self.hfname)) 

    def reproject_HMI_newOBS(self,NewFOV_xy_width = [200,200],HMI_Mag = None,New_Obs_dist = None,Plot = False, FOV_centre = None,FOV_centre_from_AIA = True, AIAcut_image = None, OutFile=None):
        '''
        ...
        if FOV_centre_from_AIA is False:
            if FOV_centre is None: 
                ...
                if FOV.shape != (2,2): Exception("%% simar_error : Incorrect FOV. It should be an 2D array [[xy_bot_left],[xy_top_right]]")
    ...
    def Project_loops_to_OBS(self,HMI_Mag = None,HMI_Mag_orig = None, ExtPo_loop_par_file = None,OutFile=None,N_loops=None, Plot=False, AIA_Image = None, PlotAIA=False,lw=2,alpha = 0.5, color = 'C0', StoreOutput = True,hmivmin = -500, hmivmax = 500):
        ...
        with fits.open(ExtPo_loop_par_file) as hdul:
            ...
            if N_loops is not None:
                if N_loops > nloops:
                     print('%simar_message: N_loops is exeeding maximum available loops. Set to maximum value of ->'+format('%d'%nloops))
                else: nloops = N_loops
            for l in range(2,nloops):
                ...
                if 'xp' in data.columns.names: 
                    ...
                    #data['flag'] = flag
                    error_stat = 1
                    print("%% simar_error : Column 'xp' exist. Update table data functionality will to be available in future!!")
                    break 
                else:
    ...
    def select_region(self,FOV=None,OutFile = None):
        '''
        ...
        if os.path.isfile(afile) is False: raise Exception("%% simar_error : AIA level-1.5 data not exist (run get_data and data_processing) - "+afile)
        hdir = os.path.join(self.config['SelectEvent']['data_dir'],'HMI')
        ...
        if os.path.isfile(hfile) is False:raise Exception("%% simar_error : HMI level-1.5 data not exist (run get_data and data_processing) - "+hfile)
        ...
        if FOV is not None:
            FOV = np.array(FOV)
            print("%% simar_message :Creating cutouts for FOV -> [["+format('%0.1f'%FOV[0][0])+','+format('%0.1f'%FOV[0][1])+'], ['+format('%0.1f'%FOV[1][0])+','+format('%0.1f'%FOV[1][1])+']]')
            if FOV.shape == (2,2):
                ...
                print("%% simar_message: Cutouts are saved.")
            else: Exception("%% simar_error : Incorrect FOV. It should be an 2D array [[xy_bot_left],[xy_top_right]]")
        else: cid = fig[0].canvas.mpl_connect('button_press_event', lambda event: select_ar_onclick(event, fig,AllAxes,out_file,a,h,outfile_dir=self.config['SelectEvent']['data_dir']))
        plt.show()
        out_file.close()
        plt.close('all') 
    ...
    def trace_fields(self,Bcube,N_lines = 10,Seed_Zheight = 0, B_pos_thresold = 10, B_neg_thresold = -10,PlotAIA = False, AIA_Image = None, OutDir = None, OutFileName = None, Seeds_file = None,HMI_Mag = None, hmivmin=-500,hmivmax=500):
        ...
        if len(Bcube.shape) != 4: raise Exception("%% simar_error: Incorrect dimension of Bcube!")
        Bx = Bcube[:,:,:,0]
        By = Bcube[:,:,:,1]
        Bz = Bcube[:,:,:,2]
        if Seed_Zheight > len(Bz[0,0,:])-1 : 
            print("%% simar_massege : Seed_Zheight is more that the extrapolated volum. Set at Z-boundery!")
        ...
        if self.NumCore == 0: #Single core
            ...
            while l_ind < N_lines:
                ...
                if l_ind > N_lines+5000 : 
                    print('simar_message: Exceeding maximum iterations for number of loops. Do you want to continue (y/n)?')
                    inp = str(input())
                    if inp == 'y': l_ind = 0
                    else : break 
    ...
    def plot_loop(self,zlim=[0,100],HMI_Mag = None, PlotAIA = True, AIA_Image=None, N_loops=None, ExtPo_loop_par_file = None, mode='2D',lw=1,color='C0',alpha=0.7,OplotSeeds=False,plot_loop_WCS = True,Proj_corr =False,Loop_indx = None):
        '''
    ...
    def get_loop_parameters(self,N_loops=None, ExtPo_loop_par_file = None,define_Bbase = 1, Min_Z = 1,OutDir = None, OutFileName = None,dxdydz_Mm = None):
        ...
        with fits.open(ExtPo_loop_par_file) as hdul:
            ...
            if nloops > len(data_all) : 
                nloops = len(data_all)
                print('%simar_massege : Exceeding maximum available loops. Set to maximum number of loops ->'+format('%d'%nloops))
            for l in range(2,nloops):
